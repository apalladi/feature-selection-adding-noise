...
def test_train_kfold_splitting():
    ...
    assert (
        len(df_coef) == features.shape[1]
    ), "The length of df_coef must match the number of features"
...
def test_scan_feature_pipeline():
    features, labels = create_regression_data()
    model = Lasso()
    reduced_features = scan_features_pipeline(
        features,
        labels,
        model,
    ...
    assert (
        reduced_features.shape[1] < features.shape[1]
    ), "The pipeline did not reduce the number of features"
...
def test_get_relevant_features():
    ...
    assert (
        x_new.shape[1] < features.shape[1]
    ), "The pipeline did not reduce the number of features"
